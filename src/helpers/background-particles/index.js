...
if (typeof window !== 'undefined') {
  ...
  _init = (canvas, window, document, options) => {
    params = {
      ...params,
      ...options,
    }
  ...
  const _Particle = function(x, y) {
    ...
    this.detectCollisions = () => {
      ...
      if (this.x + radius > W - params.margin)
        this.x = W - radius - params.margin
      else if (this.x - radius < 0 + params.margin)
        this.x = radius + params.margin

      if (this.y + radius > H - params.margin)
        this.y = H - radius - params.margin
    ...
    this.randomPosition = () => {
      ...
      while (true) {
        ...
        if (collides === false) {
          if (DEBUG)
            console.log(
              `Found a suitable location after ${attempts}  attempts.`
            )
          break
        }
